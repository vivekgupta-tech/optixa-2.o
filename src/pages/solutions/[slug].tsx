        ...
        <div className="container mx-auto px-4 md:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {solution.challenges.map((challenge, i) => (
              <ScrollReveal key={i} delay={i * 0.1} className="bg-white/5 border border-white/10 p-8">
                <h3 className="text-2xl font-bold mb-4 text-primary">{challenge.title}</h3>
                <p className="text-gray-400 text-lg leading-relaxed">{challenge.description}</p>
              </ScrollReveal>
            ))}
          </div>
        </div>
            ...
            {solution.features.map((feat, i) => {
              ...
              return (
                <ScrollReveal key={i} className={cn("flex flex-col md:flex-row gap-12 items-center", !isEven && "md:flex-row-reverse")}>
                  <div className="w-full md:w-1/2 bg-white aspect-video flex items-center justify-center border border-gray-200 shadow-sm rounded-xl">
                    <Box className="w-24 h-24 text-gray-200" />
                  </div>
                  <div className="w-full md:w-1/2">
                    <h3 className="text-3xl font-black text-black mb-4">{feat.title}</h3>
                    <p className="text-xl text-gray-600 leading-relaxed">{feat.description}</p>
                  </div>
                </ScrollReveal>
              );
            })}
          ...
          <div className="grid md:grid-cols-3 gap-12 max-w-5xl mx-auto text-center">
            {solution.benefits.map((benefit, i) => (
              <ScrollReveal key={i} delay={i * 0.1}>
                ...
                <h4 className="text-2xl font-bold text-black uppercase tracking-wide">{benefit.label}</h4>
              </ScrollReveal>
            ))}
          </div>
        </div>
